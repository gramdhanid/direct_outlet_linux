...
void change_led_state(int switch_state)
{
    int active_level = LED_DARK_HIGH;
    if (switch_state == SWITCH_OFF)
    {
        gpio_set_level(GPIO_OUTPUT_MAINLED, active_level);
    }
    else
    {
        gpio_set_level(GPIO_OUTPUT_MAINLED, MAINLED_GPIO_ON & ~active_level);
    }
}
...
void change_switch_state(int switch_state)
{
    ...
    if (switch_state == SWITCH_OFF)
    {
        gpio_set_level(GPIO_OUTPUT_RELAY, active_level);
    }
    else
    {
        gpio_set_level(GPIO_OUTPUT_RELAY, MAINLED_GPIO_ON & ~active_level);
    }
}
...
void led_blink(int led_state, int delay, int count)
{
    for (int i = 0; i < count; i++)
    {
        vTaskDelay(delay / portTICK_PERIOD_MS);
        change_led_state(1 - led_state);
        vTaskDelay(delay / portTICK_PERIOD_MS);
...
void iot_gpio_init(void)
{
    ...
    io_conf.pin_bit_mask = bit << GPIO_OUTPUT_MAINLED;
    ...
    io_conf.intr_type = GPIO_INTR_DISABLE;
    io_conf.mode = GPIO_MODE_OUTPUT;
    io_conf.pin_bit_mask = bit << GPIO_OUTPUT_RELAY;
    io_conf.pull_down_en = 0;
    io_conf.pull_up_en = 1;
    gpio_config(&io_conf);
