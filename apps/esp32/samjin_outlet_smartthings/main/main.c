...
static float last_energy;
// static float quota_energy;
...
//         // unsigned int task_delay_sec = 300;
...
static void capability_init()
{
    ...
    cap_powerMeter_data = caps_powerMeter_initialize(iot_ctx, "main", NULL, NULL);
    cap_energyMeter_data = caps_energyMeter_initialize(iot_ctx, "main", NULL, NULL);
    ...
    if (cap_powerMeter_data)
    {
        const double power_init_value = caps_helper_powerMeter.attr_power.min;
        const char *power_unit = caps_helper_powerMeter.attr_power.unit_W;

        cap_powerMeter_data->set_power_value(cap_powerMeter_data, power_init_value);
        cap_powerMeter_data->set_power_unit(cap_powerMeter_data, power_unit);
    }

    if (cap_energyMeter_data)
    {
        const double energy_init_value = caps_helper_energyMeter.attr_energy.min;
        const char *energy_unit = caps_helper_energyMeter.attr_energy.unit_kWh;

        cap_energyMeter_data->set_energy_value(cap_energyMeter_data, energy_init_value);
        cap_energyMeter_data->get_energy_unit(cap_energyMeter_data);
        cap_energyMeter_data->set_energy_unit(cap_energyMeter_data, energy_unit);
...
static bool power_event_need_update(void)
{
    float power = cotx->meter.power;
    if (power < 10)
    {
        if (fabs(last_power - power) > 0.1f)
            return true;
    }
    else if ((power > 10) && (power < 100))
    {
        if (fabs(last_power - power) > 1)
            return true;
    }
    else if (power > 100)
    {
        if (fabs(last_power - power) > 10)
            return true;
    }
    return false;
}
// static bool energy_event_need_update(void)
// {
//     float power = cotx->meter.power;
//     // float energy = context->meter.energy;
//     quota_energy = quota_energy + power;
//     if (fabs(last_power - power) > (power / 10))
//     {
//         return true;
//     }
//     else if ((quota_energy > (power * 200)) && (power > 0))
//     {
//         quota_energy = 0;
//         return true;
//     }
...
void power_update()
{
    cap_powerMeter_data->set_power_value(cap_powerMeter_data, cotx->meter.power);
    cap_powerMeter_data->attr_power_send(cap_powerMeter_data);
    cap_energyMeter_data->set_energy_value(cap_energyMeter_data, cotx->meter.energy);
    cap_energyMeter_data->attr_energy_send(cap_energyMeter_data);
    last_power = cotx->meter.power;
    last_energy = cotx->meter.energy;
}
...
static void app_main_task(void *arg)
{
    ...
    for (;;)
    {
        ...
        if (power_event_need_update())
        {
            power_update();
        }
        // if (energy_event_need_update())
        // {
        //     energy_update();
        // }
